...
void test_cutoff_cos(){
	...
	Basis basis(rc,cutoff::Name::COS,cutoff::Norm::UNIT,1);
	//compute error
...
void test_unit_nnh(){
	...
	init.seed()=-1;
	init.tf()=NN::Transfer::ARCTAN;
	nnh.nn().resize(init,nh);
	//initialize basis
	std::cout<<"initializing basis\n";
	const int nR=6;
	BasisR basisR(rc,cutoff::Name::COS,cutoff::Norm::UNIT,nR,PhiRN::TANH);
	const int nA=4;
	BasisA basisA(rc,cutoff::Name::COS,cutoff::Norm::UNIT,nA,PhiAN::GAUSS);
	for(int i=0; i<nR; ++i){
		basisR.rs(i)=1.06;
		basisR.eta(i)=i;
	}
...
void test_unit_symm(){
	...
	init.init()=NN::Init::HE;
	init.seed()=-1;
	init.tf()=NN::Transfer::SWISH;
	//initialize basis
	std::cout<<"initializing basis\n";
	const int nR=6;
	BasisR basisR(rcut,cutoff::Name::COS,cutoff::Norm::UNIT,nR,PhiRN::TANH);
	const int nA=4;
	BasisA basisA(rcut,cutoff::Name::COS,cutoff::Norm::UNIT,nA,PhiAN::GAUSS);
	for(int i=0; i<nR; ++i){
		basisR.rs(i)=1.06;
		basisR.eta(i)=i;
	}
