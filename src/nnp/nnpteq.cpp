...
template <> int unpack(NNPTEQ& obj, const char* arr){
	...
	//optimization
		...
		obj.delta2_=obj.delta_*obj.delta_;
	//return bytes read
		return pos;
}
...
void NNPTEQ::train(int batchSize, std::vector<Structure>& struc_train, std::vector<Structure>& struc_val){
	...
	if(WORLD.rank()==0){
		...
		if(writer_error_==NULL) throw std::runtime_error("NNPTEQ::train(int): Could not open error record file.");
...
double NNPTEQ::error(const Eigen::VectorXd& x, std::vector<Structure>& struc_train, std::vector<Structure>& struc_val){
	...
	for(int i=0; i<batch_.size(); ++i){
		...
		switch(obj_.loss()){
			...
			case opt::Loss::MAE:{
				error_train_+=fabs(dE);
				gpre=math::func::sgn(dE)*nAtomsI;
			}break;
			case opt::Loss::HUBER:{
				const double rad=sqrt(dE*dE/delta2_+1.0);
				error_train_+=delta2_*(rad-1.0);
				gpre=dE*nAtomsI/rad;
			}break;
			case opt::Loss::LOGCOSH:{
				const double arg=dE/delta_;
				if(dE>0.0){
					const double expf=exp(-2.0*arg);
					error_train_+=delta2_*(math::special::logp1(expf)+arg-LOG2);
					gpre=(1.0-expf)/(1.0+expf)*delta_*nAtomsI;
				} else {
					const double expf=exp(2.0*arg);
					error_train_+=delta2_*(math::special::logp1(expf)-arg-LOG2);
					gpre=(expf-1.0)/(expf+1.0)*delta_*nAtomsI;
				}
			}break;
			case opt::Loss::LOGP1:{
				const double arg=dE/delta_;
				if(arg>0.0){
					const double logf=log1p(arg);
					error_train_+=delta2_*(-arg+(arg+1.0)*logf);
					gpre=logf*delta_*nAtomsI;
				} else {
					const double logf=log1p(-arg);
					error_train_+=delta2_*(arg-(arg-1.0)*logf);
					gpre=logf*delta_*nAtomsI*-1.0;
				}
			}break;
			default:{
				gpre=0;
		...
		for(int n=0; n<dist_atomt[ii].size(); ++n){
			...
			dcda<<gpre,0.5*gpre*struc_train[ii].charge(m);
			//compute gradient
		...
		for(int i=0; i<struc_val.size(); ++i){
			...
			switch(obj_.loss()){
				...
				case opt::Loss::LOGCOSH:{
					const double arg=dE/delta_;
					if(arg>0.0){
						error_val_+=delta2_*(math::special::logp1(exp(-2.0*arg))+arg-LOG2);
					} else {
						error_val_+=delta2_*(math::special::logp1(exp(2.0*arg))-arg-LOG2);
					}
				} break;
				case opt::Loss::LOGP1:{
					const double arg=dE/delta_;
					if(arg>0.0){
						error_val_+=delta2_*(-arg+(arg+1.0)*log1p(arg));
					} else {
						error_val_+=delta2_*(arg-(arg-1.0)*log1p(-arg));
					}
				} break;
	...
	//====== return the error ======
	if(NNPTEQ_PRINT_STATUS>0 && WORLD.rank()==0) std::cout<<"returning the error\n";
	return error_train_;
}
...
int main(int argc, char* argv[]){
	...
	try{
		...
		dist[0].init(BATCH.ncomm(),BATCH.color(),nstrucs[0]);
		dist[1].init(BATCH.ncomm(),BATCH.color(),nstrucs[1]);
		dist[2].init(BATCH.ncomm(),BATCH.color(),nstrucs[2]);
		dist[3].init(BATCH.ncomm(),BATCH.color(),nBatch);
		//print
		if(WORLD.rank()==0){
			std::string str;
		...
		//======== read the structures ========
		if(WORLD.rank()==0 && NNPTEQ_PRINT_STATUS>-1) std::cout<<"reading structures\n";
		for(int n=0; n<3; ++n){
			if(files[n].size()>0){
			...
			if(dist[ii].size()>0){
				...
				for(int nn=0; nn<dist[ii].size(); ++nn){
					for(int n=0; n<strucs[ii][nn].nAtoms(); ++n){
						...
						//execute the network
						nnpteq.nnp().nnh(type).nn().execute(strucs[ii][nn].symm(n));
