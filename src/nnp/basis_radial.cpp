...
#include "src/nnp/symm_radial_g1.hpp"
#include "src/nnp/symm_radial_g2.hpp"
#include "src/nnp/symm_radial_t1.hpp"
// ann - str
...
*/
BasisR::BasisR(const BasisR& basis):normT_(cutoff::Norm::UNKNOWN),phiRN_(PhiRN::UNKNOWN),fR_(NULL),cutoff_(NULL){
	...
	normT_=basis.normT();//normalization scheme
...
std::ostream& operator<<(std::ostream& out, const BasisR& basis){
	if(basis.cutoff()!=NULL) out<<"BasisR "<<basis.norm_<<" "<<basis.cutoff_->name()<<" "<<basis.rc_<<" "<<basis.phiRN_<<" "<<basis.nfR_;
...
*/
void BasisR::init_G1(int nR, cutoff::Norm normT, cutoff::Name tcut, double rcut){
	if(BASIS_RADIAL_PRINT_FUNC>0) std::cout<<"BasisR::init_G1(int,cutoff::Name,double,double):\n";
	if(rcut<=0) throw std::invalid_argument("BasisR::init_G1(int,cutoff::Name,double): Invalid radial cutoff.");
	clear();
...
* @param rcut - the cutoff distance
*/
void BasisR::init_G2(int nR, cutoff::Norm normT, cutoff::Name tcut, double rcut){
	if(BASIS_RADIAL_PRINT_FUNC>0) std::cout<<"BasisR::init_G2(int,cutoff::Name,double):\n";
	if(rcut<=0) throw std::invalid_argument("BasisR::init_G2(int,cutoff::Name,double): Invalid radial cutoff.");
	clear();
	//set basis parameters
	normT_=normT;//normalization scheme
...
* @param rcut - the cutoff distance
*/
void BasisR::init_T1(int nR, cutoff::Norm normT, cutoff::Name tcut, double rcut){
	if(BASIS_RADIAL_PRINT_FUNC>0) std::cout<<"BasisR::init_T1(int,cutoff::Name,double):\n";
	if(rcut<=0) throw std::invalid_argument("BasisR::init_T1(int,cutoff::Name,double): Invalid radial cutoff.");
	clear();
	//set basis parameters
	normT_=normT;//normalization scheme
...
*/
double BasisR::norm(cutoff::Norm normT, double rc){
	double tmp=0;
	switch(normT){
...
namespace serialize{
	...
	template <> int nbytes(const BasisR& obj){
		...
		switch(obj.phiRN()){
			case PhiRN::G1: for(int i=0; i<obj.nfR(); ++i) N+=nbytes(static_cast<const PhiR_G1&>(obj.fR(i))); break;
			case PhiRN::G2: for(int i=0; i<obj.nfR(); ++i) N+=nbytes(static_cast<const PhiR_G2&>(obj.fR(i))); break;
			case PhiRN::T1: for(int i=0; i<obj.nfR(); ++i) N+=nbytes(static_cast<const PhiR_T1&>(obj.fR(i))); break;
			default: break;
		}
		return N;
	}
	...
	template <> int pack(const BasisR& obj, char* arr){
		...
		switch(obj.phiRN()){
			case PhiRN::G1: for(int i=0; i<obj.nfR(); ++i) pos+=pack(static_cast<const PhiR_G1&>(obj.fR(i)),arr+pos); break;
			case PhiRN::G2: for(int i=0; i<obj.nfR(); ++i) pos+=pack(static_cast<const PhiR_G2&>(obj.fR(i)),arr+pos); break;
			case PhiRN::T1: for(int i=0; i<obj.nfR(); ++i) pos+=pack(static_cast<const PhiR_T1&>(obj.fR(i)),arr+pos); break;
			default: break;
		}
		return pos;
	}
	...
	template <> int unpack(BasisR& obj, const char* arr){
		...
		switch(phiRN){
			case PhiRN::G1: obj.init_G1(nfR,normT,cutN,rc); for(int i=0; i<obj.nfR(); ++i) pos+=unpack(static_cast<PhiR_G1&>(obj.fR(i)),arr+pos); break;
			case PhiRN::G2: obj.init_G2(nfR,normT,cutN,rc); for(int i=0; i<obj.nfR(); ++i) pos+=unpack(static_cast<PhiR_G2&>(obj.fR(i)),arr+pos); break;
			case PhiRN::T1: obj.init_T1(nfR,normT,cutN,rc); for(int i=0; i<obj.nfR(); ++i) pos+=unpack(static_cast<PhiR_T1&>(obj.fR(i)),arr+pos); break;
			default: break;
		}
		return pos;
	}
