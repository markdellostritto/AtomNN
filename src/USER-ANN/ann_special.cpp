...
namespace special{
	...
	double erfa2(double x){double s=func::sign(x); x*=s; return s*(1.0-func::poly<3>(1.0/(1.0+erfa_const::a2[4]*x),erfa_const::a2)*exp(-x*x));}
	double erfa3(double x){double s=func::sign(x); x*=s; x=func::poly<6>(x,erfa_const::a3); x*=x; x*=x; x*=x; x*=x; return s*(1.0-1.0/x);}
	double erfa4(double x){double s=func::sign(x); x*=s; return s*(1.0-func::poly<5>(1.0/(1.0+erfa_const::a4[6]*x),erfa_const::a4)*exp(-x*x));}
