...
#include "ann_symm_angular_g3.h"
#include "ann_symm_angular_g4.h"
// ann - serialization
...
void PairNN::compute(int eflag, int vflag){
	...
	for(int ii=0; ii<inum; ++ii){
		...
		for(int jj=0; jj<num_nn; ++jj){
			...
			const Eigen::Vector3d rIJ=(Eigen::Vector3d()<<x[i][0]-x[j][0],x[i][1]-x[j][1],x[i][2]-x[j][2]).finished();
			...
			if(dIJ<rc_){
				...
				for(int kk=jj+1; kk<num_nn; ++kk){
					...
					const Eigen::Vector3d rIK=(Eigen::Vector3d()<<x[i][0]-x[k][0],x[i][1]-x[k][1],x[i][2]-x[k][2]).finished();
					const double dIK=rIK.norm();//compute norm
					if(dIK<rc_){
						//compute dJK
						//rJK<<x[j][0]-x[k][0],x[j][1]-x[k][1],x[j][2]-x[k][2];
						const Eigen::Vector3d rJK=(Eigen::Vector3d()<<x[j][0]-x[k][0],x[j][1]-x[k][1],x[j][2]-x[k][2]).finished();
