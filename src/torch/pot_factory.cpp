...
#include "src/torch/pot_coul_wolf.hpp"
#include "src/torch/pot_coul_dsf.hpp"
...
#include "src/torch/pot_qeq_gl.hpp"
#include "src/torch/pot_nnpe.hpp"
#include "src/torch/pot_nnpeq.hpp"
...
namespace ptnl{
	...
	std::shared_ptr<Pot>& read(std::shared_ptr<Pot>& pot, Token& token){
		...
		switch(name){
			...
			case Pot::Name::NNPEQ:{
				pot.reset(new PotNNPEQ());
				static_cast<PotNNPEQ&>(*pot).read(token);
			}break;
			default:{
	...
	std::ostream& operator<<(std::ostream& out, const std::shared_ptr<Pot>& pot){
		switch(pot->name()){
			...
			case Pot::Name::NNPEQ: out<<static_cast<const PotNNPEQ&>(*pot); break;
			default: break;
		}
		return out;
	}
	...
	template <> int nbytes(const std::shared_ptr<ptnl::Pot>& obj){
		...
		if(obj!=nullptr){
			...
			switch(obj->name()){
				...
				case ptnl::Pot::Name::NNPEQ: size+=nbytes(static_cast<const ptnl::PotNNPEQ&>(*obj)); break;
	...
	template <> int pack(const std::shared_ptr<ptnl::Pot>& obj, char* arr){
		...
		if(!null){
			...
			switch(obj->name()){
				...
				case ptnl::Pot::Name::NNPEQ: pos+=pack(static_cast<const ptnl::PotNNPEQ&>(*obj),arr+pos); break;
	...
	template <> int unpack(std::shared_ptr<ptnl::Pot>& obj, const char* arr){
		...
		if(!null){
			...
			switch(obj->name()){
				...
				case ptnl::Pot::Name::NNPEQ: pos+=unpack(static_cast<ptnl::PotNNPEQ&>(*obj),arr+pos); break;
