	...
	for(int i=0; i<struc.nAtoms(); ++i){
		...
		for(int j=0; j<nlist.size(i); ++j){
			...
			if(nlist.neigh(i,j).dr()<rc_){
				...
				const double b=s6_(ti,tj)*dri6;
				energy_+=4.0*e_(ti,tj)*b*(b-1.0);
			}
		}
	}
	energy_*=0.5;
	...
	for(int i=0; i<struc.nAtoms(); ++i){
		...
		for(int j=0; j<nlist.size(i); ++j){
			...
			if(nlist.neigh(i,j).dr()<rc_){
				...
				const double dri3=dri*dri*dri;
				const double dri6=dri3*dri3;
				const double b=s6_(ti,tj)*dri6;//(sigma/r)^6
				energy_+=4.0*e_(ti,tj)*b*(b-1.0);
				const Eigen::Vector3d fij=24.0*e_(ti,tj)*b*(2.0*b-1.0)*dri*dri*r;
