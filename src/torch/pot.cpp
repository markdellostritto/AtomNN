...
Pot::Name Pot::Name::read(const char* str){
	if(std::strcmp(str,"LJ_CUT")==0) return Pot::Name::LJ_CUT;
	...
	else if(std::strcmp(str,"NNPEQ")==0) return Pot::Name::NNPEQ;
...
const char* Pot::Name::name(const Pot::Name& t){
	switch(t){
		...
		case Pot::Name::NNPEQ: return "NNPEQ";
...
std::ostream& operator<<(std::ostream& out, const Pot::Name& t){
	switch(t){
		...
		case Pot::Name::NNPEQ: out<<"NNPEQ"; break;
