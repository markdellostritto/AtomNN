...
const char* Algo::name(const Algo& algo){
	switch(algo){
...
Decay Decay::read(const char* str){
...
double Loss::error(Loss loss, const Eigen::VectorXd& value, const Eigen::VectorXd& target){
	if(OPT_PRINT_FUNC>0) std::cout<<"Loss::error(const Eigen::VectorXd&,const Eigen::VectorXd&):\n";
	//compute the error
	double err=0;
	switch(loss){
		case Loss::MSE:{
...
double Loss::error(Loss loss, const Eigen::VectorXd& value, const Eigen::VectorXd& target, Eigen::VectorXd& grad){
	if(OPT_PRINT_FUNC>0) std::cout<<"Loss::error(const Eigen::VectorXd&,const Eigen::VectorXd&,Eigen::VectorXd&):\n";
	//compute the error
	double err=0;
	switch(loss){
		case Loss::MSE:{
...
std::ostream& Model::print(std::ostream& out, const Model* model){
	switch(model->algo()){
		case Opt::Algo::SGD:      out<<static_cast<const Opt::SGD&>(*model)<<"\n"; break;
		case Opt::Algo::SDM:      out<<static_cast<const Opt::SDM&>(*model)<<"\n"; break;
		case Opt::Algo::ADAGRAD:  out<<static_cast<const Opt::ADAGRAD&>(*model)<<"\n"; break;
		case Opt::Algo::ADADELTA: out<<static_cast<const Opt::ADADELTA&>(*model)<<"\n"; break;
		case Opt::Algo::RMSPROP:  out<<static_cast<const Opt::RMSPROP&>(*model)<<"\n"; break;
		case Opt::Algo::ADAM:     out<<static_cast<const Opt::ADAM&>(*model)<<"\n"; break;
		case Opt::Algo::NADAM:    out<<static_cast<const Opt::NADAM&>(*model)<<"\n"; break;
		case Opt::Algo::AMSGRAD:  out<<static_cast<const Opt::AMSGRAD&>(*model)<<"\n"; break;
		case Opt::Algo::BFGS:     out<<static_cast<const Opt::BFGS&>(*model)<<"\n"; break;
		case Opt::Algo::RPROP:    out<<static_cast<const Opt::RPROP&>(*model)<<"\n"; break;
		default: out<<"INVALID\n";
	}
	return out;
}
...
Data& read(Data& data, FILE* reader){
	...
	while(fgets(input,string::M,reader)!=NULL){
		...
		} else if(strlist.at(0)=="STOP"){
			data.stop()=Stop::read(string::to_upper(strlist.at(1)).c_str());
...
template <> int pack(const Opt::Data& obj, char* arr){
	...
	//algorithm
		std::memcpy(arr+pos,&obj.algo(),sizeof(Opt::Algo)); pos+=sizeof(Opt::Algo);//algo_
...
template <> int pack(const Opt::Model& obj, char* arr){
	...
	std::memcpy(arr+pos,&obj.algo(),sizeof(Opt::Algo)); pos+=sizeof(Opt::Algo);//algo_
	std::memcpy(arr+pos,&obj.decay(),sizeof(Opt::Decay)); pos+=sizeof(Opt::Decay);//decay_
...
template <> int unpack(Opt::Data& obj, const char* arr){
	...
	//algorithm
		std::memcpy(&obj.algo(),arr+pos,sizeof(Opt::Algo)); pos+=sizeof(Opt::Algo);//algo_
...
template <> int unpack(Opt::Model& obj, const char* arr){
	...
	std::memcpy(&obj.algo(),arr+pos,sizeof(Opt::Algo)); pos+=sizeof(Opt::Algo);//algo_
	std::memcpy(&obj.decay(),arr+pos,sizeof(Opt::Decay)); pos+=sizeof(Opt::Decay);//decay_
