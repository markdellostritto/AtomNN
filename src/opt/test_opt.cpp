...
void opt_rosen(opt::algo::Name name){
	...
	for(int i=0; i<obj.max(); ++i){
		...
		switch(obj.stop()){
			case opt::Stop::FREL: fbreak=obj.dv()<obj.tol(); break;
			case opt::Stop::XREL: fbreak=obj.dp()<obj.tol(); break;
			case opt::Stop::FABS: fbreak=obj.val()<obj.tol(); break;
		}
		if(fbreak) break;
		//print the status
