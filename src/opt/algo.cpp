...
void ADAM::step(Objective& obj){
	const Eigen::VectorXd& g=obj.g();
	//compute step
	const double fac=obj.gamma()*sqrt(1.0-beta2i_)/(1.0-beta1i_);
	for(int n=0; n<dim_; ++n){
		...
		//calculate the new position
		obj.p()[n]-=fac*mgrad_[n]/(sqrt(mgrad2_[n])+eps_);
	}
...
void NADAM::step(Objective& obj){
	...
	const double fac=obj.gamma()*sqrt(1.0-beta2i_)/(1.0-beta1i_);
	/*
	mgrad_*=beta1_; mgrad_.noalias()+=(1.0-beta1_)*g;
	mgrad2_*=beta2_; mgrad2_.noalias()+=(1.0-beta2_)*g.cwiseProduct(g);
	obj.p().noalias()-=fac*((beta1_*mgrad_.array()+(1.0-beta1_)*g.array())/(mgrad2_.array().sqrt()+eps_)).matrix();
	*/
	for(int n=0; n<dim_; ++n){
		//add to the running average of the gradients
		mgrad_[n]*=beta1_; mgrad_[n]+=(1.0-beta1_)*g[n];
		...
		mgrad2_[n]*=beta2_; mgrad2_[n]+=(1.0-beta2_)*g[n]*g[n];
		//calculate the new position
		obj.p()[n]-=fac*(beta1_*mgrad_[n]+(1.0-beta1_)*g[n])/(sqrt(mgrad2_[n])+eps_);
	}
...
void AMSGRAD::step(Objective& obj){
	...
	for(int n=0; n<dim_; ++n){
		...
		//calculate the new position
		obj.p()[n]-=fac*mgrad_[n]/(sqrt(mgrad2m_[n])+eps_);
	}
	//update the powers of betas
	beta1i_*=beta1_;
	beta2i_*=beta2_;
