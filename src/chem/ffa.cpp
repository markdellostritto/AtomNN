...
int main(int argc, char* argv[]){
	...
	//structure
		...
		Structure struc;
		std::vector<molecule::Type> atom_types;
	...
	//molecule
		...
		std::vector<molecule::Type> types;
		std::vector<molecule::Alias> aliases;
		std::vector<std::pair<std::string,double> > charges;
		std::vector<std::pair<std::string,double> > radii;
		std::vector<molecule::Coeff> pair_coeffs;
		std::vector<molecule::Coeff> bond_coeffs;
		std::vector<molecule::Coeff> angle_coeffs;
		std::vector<molecule::Coeff> dihedral_coeffs;
		std::vector<molecule::Coeff> improper_coeffs;
		std::vector<molecule::Link> pairs;
		std::vector<molecule::Link> bonds;
		std::vector<molecule::Link> angles;
		std::vector<molecule::Link> dihedrals;
		std::vector<molecule::Link> impropers;
	...
	try{
		...
		while(fgets(input,string::M,reader)!=NULL){
			...
			if(tag=="STRUC"){
				...
				types.push_back(molecule::Type());
				molecule::Type::read(token,types.back());
			} else if(tag=="ALIAS"){
				aliases.push_back(molecule::Alias());
				molecule::Alias::read(token,aliases.back());
			} else if(tag=="CHARGE"){
				...
				pair_coeffs.push_back(molecule::Coeff());
				molecule::Coeff::read(token,pair_coeffs.back());
			} else if(tag=="BOND_COEFF"){
				bond_coeffs.push_back(molecule::Coeff());
				molecule::Coeff::read(token,bond_coeffs.back());
			} else if(tag=="ANGLE_COEFF"){
				angle_coeffs.push_back(molecule::Coeff());
				molecule::Coeff::read(token,angle_coeffs.back());
			} else if(tag=="DIHEDRAL_COEFF"){
				dihedral_coeffs.push_back(molecule::Coeff());
				molecule::Coeff::read(token,dihedral_coeffs.back());
			} else if(tag=="IMPROPER_COEFF"){
				improper_coeffs.push_back(molecule::Coeff());
				molecule::Coeff::read(token,improper_coeffs.back());
			} else if(tag=="PAIR"){
				pairs.push_back(molecule::Link());
				molecule::Link::read(token,pairs.back());
			} else if(tag=="BOND"){
				bonds.push_back(molecule::Link());
				molecule::Link::read(token,bonds.back());
			} else if(tag=="ANGLE"){
				angles.push_back(molecule::Link());
				molecule::Link::read(token,angles.back());
			} else if(tag=="DIHEDRAL"){
				dihedrals.push_back(molecule::Link());
				molecule::Link::read(token,dihedrals.back());
			} else if(tag=="IMPROPER"){
				impropers.push_back(molecule::Link());
				molecule::Link::read(token,impropers.back());
			} else if(tag=="PRINT"){
		...
		molecule::make_graph(struc,molgraph);
		...
		for(int i=0; i<struc.nAtoms(); ++i){
			...
			//make lsmiles vector
			molecule::make_lsmiles(struc,molgraph,i,2,lsmilesv);
			//pack vector
				...
				for(int i0=0; i0<molgraph.edges(ii1).size(); ++i0){
					...
					for(int i2=0; i2<molgraph.edges(ii1).size(); ++i2){
						...
						for(int i3=0; i3<molgraph.edges(ii2).size(); ++i3){
							...
							/*
							std::cout<<"\t\t\t\tstr = "<<
							atom_types[ii0].label()<<" "<<
							atom_types[ii1].label()<<" "<<
							atom_types[ii2].label()<<" "<<
							atom_types[ii3].label()<<" "<<"\n";
							*/
		...
		/*
		if(dihedrals.size()>0){
			std::cout<<"computing dihedrals\n";
			for(int i0=0; i0<struc.nAtoms(); ++i0){
				const int ii0=i0;
				Graph::color_path(molgraph,ii0,4);
				//std::cout<<"path 0 atom "<<ii0+1<<" "<<struc.name(ii0)<<" "<<molgraph.node(ii0).color()<<"\n";
				for(int i1=0; i1<molgraph.edges(ii0).size(); ++i1){
					const int ii1=molgraph.edge(ii0,i1).end();
					//std::cout<<"\tpath 1 atom "<<ii1+1<<" "<<struc.name(ii1)<<" "<<molgraph.node(ii1).color()<<"\n";
					if(molgraph.node(ii1).color()<=molgraph.node(ii0).color()) continue;
					for(int i2=0; i2<molgraph.edges(ii1).size(); ++i2){
						const int ii2=molgraph.edge(ii1,i2).end();
						//std::cout<<"\t\tpath 2 atom "<<ii2+1<<" "<<struc.name(ii2)<<" "<<molgraph.node(ii2).color()<<"\n";
						if(molgraph.node(ii2).color()<=molgraph.node(ii1).color()) continue;
						for(int i3=0; i3<molgraph.edges(ii2).size(); ++i3){
							const int ii3=molgraph.edge(ii2,i3).end();
							//std::cout<<"\t\t\tpath 3 atom "<<ii3+1<<" "<<struc.name(ii3)<<" "<<molgraph.node(ii3).color()<<"\n";
							if(molgraph.node(ii3).color()<=molgraph.node(ii2).color()) continue;
							std::cout<<"\t\t\t\tstr = "<<
							atom_types[ii0].label()<<" "<<
							atom_types[ii1].label()<<" "<<
		...
		*/
		...
		//==== label the graph by connected component (molecule) ====
